...
def enviar_telegram(mensaje, thread_id):
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Error Telegram Celador (Hilo {thread_id}): {response.text}")
...
def procesar_gerencia(session, nombre, valor_gerencia, thread_id):
    ...
    try:
        ...
        print(f"[{nombre}] Filas Celador detectadas: {len(filas)}")
        ...
        print(f"Error procesando Celador en {nombre}: {e}")
