...
from zoneinfo import ZoneInfo  # Zona horaria nativa
...
# Google Sheet exclusivo para Celador
SPREADSHEET_ID = "1h7HhbHqa_2_KGkaulDRKQzSpoiyHSuMUqI8Hf7QSXC0"

# ID oficial para Celador
CATEGORIA_CELADOR = "6"
...
def registrar_en_sheets(
    ...
    try:
        ...
        # Un fallo de Sheets no debe impedir que continúe
        # funcionando el scraper ni Telegram.
...
def enviar_telegram(mensaje, thread_id):
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"Error Telegram Celador "
                f"(Hilo {thread_id}): {response.text}"
            )

    except Exception as e:
        print(f"Error sending to Telegram Celador: {e}")
...
def procesar_gerencia(
    ...
    fichero_estado = f"estado_celador_{valor_gerencia}.txt"
    ...
    try:
        ...
        payload_c = {
            "j_idt13": "j_idt13",
            "j_idt13:categoriasSOM_input": CATEGORIA_CELADOR,
        ...
        print(
            f"[{nombre}] Filas Celador detectadas: {len(filas)}"
        )
        ...
        print(
            f"Error procesando Celador en {nombre}: {e}"
        )
